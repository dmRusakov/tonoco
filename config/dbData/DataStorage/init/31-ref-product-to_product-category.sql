...
-- auto set "order" column by product_category_id (Order of product in product category)
...
BEGIN
    ...
    SET "order" = (SELECT COUNT(*) FROM public.ref_product_to_product_category WHERE product_category_id = NEW.product_category_id) + 1
    WHERE id = NEW.id;
    RETURN NEW;
END;
$BODY$;
