...
type Item = entity.SpecificationValue
type Filter = entity.SpecificationValueFilter
...
func (m *Model) makeStatementByFilter(filter *Filter) sq.SelectBuilder {
	...
	// SpecificationIDs
	if filter.SpecificationIDs != nil {
		countSpecificationIDs := len(*filter.SpecificationIDs)

		if countSpecificationIDs > 0 {
			statement = statement.Where(sq.Eq{fieldMap["SpecificationID"]: *filter.SpecificationIDs})
		}

		*filter.Page = 1
		if (*filter.PerPage) > uint64(countSpecificationIDs) {
			*filter.PerPage = uint64(countSpecificationIDs)
		}
	}
...
func (m *Model) makeUpdateStatement(ctx context.Context, item *Item) sq.UpdateBuilder {
	...
	return m.qb.Update(m.table).
		Set(fieldMap["SpecificationID"], item.SpecificationID).
