...
type Item = entity.SpecificationType
type Filter = entity.SpecificationTypeFilter
...
func (m *Model) makeStatementByFilter(filter *Filter) sq.SelectBuilder {
	...
	// Unit
	if filter.Unit != nil {
		statement = statement.Where(sq.Eq{fieldMap["Unit"]: *filter.Unit})
	}
