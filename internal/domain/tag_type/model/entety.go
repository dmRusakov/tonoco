...
func (m *Model) makeStatementByFilter(filter *Filter) sq.SelectBuilder {
	...
	if filter.ListItem != nil {
		statement = statement.Where(sq.Eq{fieldMap["ListItem"]: *filter.ListItem})
	}

	// Filter
	if filter.Filter != nil {
		statement = statement.Where(sq.Eq{fieldMap["Filter"]: *filter.Filter})
	}

	// Type
	if filter.Type != nil {
		statement = statement.Where(sq.Eq{fieldMap["Type"]: *filter.Type})
	}
...
func (m *Model) makeUpdateStatement(ctx context.Context, item *Item) sq.UpdateBuilder {
	...
	return m.qb.Update(m.table).
		...
		Set(fieldMap["Required"], item.Required).
		Set(fieldMap["Active"], item.Active).
		Set(fieldMap["Prime"], item.Prime).
		Set(fieldMap["ListItem"], item.ListItem).
		Set(fieldMap["Filter"], item.Filter).
		Set(fieldMap["SortOrder"], item.SortOrder).
		Set(fieldMap["Type"], item.Type).
		Set(fieldMap["Prefix"], item.Prefix).
		Set(fieldMap["Suffix"], item.Suffix).
